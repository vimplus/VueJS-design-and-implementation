...
* 双端 Diff 算法的原理：在新旧两组子节点的四个端点之间分别进行比较，并试图找到可复用的节点。（主要分为四步：新头旧头、新尾旧尾、新尾旧头、新头旧尾）
* 双端 Diff 算法就是**记录新旧两个子数组的四个端点**，通过：新头节点-旧头结点、新尾结点-旧尾结点、旧头结点-新尾结点、旧尾结点-新头节点，这样四种组合依次去比较，直到找到匹配的元素，然后根据新节点的位置把对应的旧节点移动。
* 双端 Diff 算法的优势：与简单 Diff 算法相比，对于同样的更新场景，执行的DOM 移动操作次数更少。
...
* [《Vue.js 设计与实现》读书笔记](https://www.cnblogs.com/wenruo/p/17047716.html)